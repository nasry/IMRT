...
http://192.168.1.205:8080/api/v1/namespaces/default/services/$app | jq  '.spec.ports[0].port = $p1 | .spec.ports[0].targetPort = $p1 | .spec.ports[1].port = $p2 | .spec.ports[1].targetPort = $p2' > harvester-auto.json;
